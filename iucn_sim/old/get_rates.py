...
def add_arguments(parser):
    ...
    # args = p.parse_args()    
    # args.input_data = '/Users/tobias/GitHub/iucn_extinction_simulator/data/example_data/gl_data_all_birds.txt'
...
def main(args):    
    ...
    if allow_precompiled_iucn_data:
        for taxon_group in taxon_reference_groups:
            if use_ex_transition_rates:
                try:
                    # look for precompiled files online    
                    url = 'https://raw.githubusercontent.com/tobiashofmann88/iucn_extinction_simulator/master/data/precompiled/iucn_history/%s_iucn_history_incl_extinct.txt'%taxon_group.upper()
                    urlpath =urlopen(url)
    ...
    # replace EX with NaN
    if not use_ex_transition_rates:
