...
plt.errorbar(true_rates,simulated_mean, yerr=np.array([simulated_delta_lower,simulated_delta_upper]), fmt='.',ecolor='black',elinewidth=1,capsize=2)
plt.plot(true_rates,true_rates,'-',color='red',zorder=10)
plt.xlim(min(true_rates)-0.5*min(true_rates),max(true_rates)+0.5*max(true_rates))
plt.ylim(min(true_rates)-0.5*min(true_rates),max(true_rates)+0.5*max(true_rates))
ax=plt.gca()
ax.set_xscale('log')
ax.set_yscale('log')
