...
5. Test if installation worked by typing `iucn_sim --version`, which should show a **verison number >= 2.1** (if you created a virtual environment, you need to be connected to it for any `iucn_sim` command to work). If this results in an error, go to the next step. If the command works but it shows an older version number (< 2.1), try `conda update -n base conda` and then `conda update iucn_sim`.
